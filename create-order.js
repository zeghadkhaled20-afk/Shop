...
export default async function handler(req, res) {
  ...
  const body = req.body || {};
  const {
    name, phone, wilaya, commune, address, phoneModel, variantId, colorLabel,
    deliveryType, shippingPrice, productPrice, totalPrice
  } = body;
  ...
  if (
    ...
    !Number.isFinite(shippingPriceNum) || shippingPriceNum < 0
  ) {
  ...
  const variables = {
    input: {
      ...
      note: `طلب من صفحة الهبوط — الدفع عند الاستلام (COD) — ${deliveryType || 'توصيل'} — ${wilaya}`,
      customAttributes: [
        { key: 'الاسم', value: name.trim() },
        { key: 'الهاتف', value: phone.trim() },
        { key: 'الولاية', value: wilaya },
        { key: 'البلدية', value: commune.trim() },
        { key: 'العنوان', value: address.trim() },
        { key: 'الطراز', value: phoneModel || '' },
        { key: 'اللون', value: colorLabel || '' },
        { key: 'طريقة الدفع', value: 'الدفع عند الاستلام' },
        { key: 'نوع التوصيل', value: deliveryType || '' },
        { key: 'سعر التوصيل', value: String(shippingPriceNum) },
        { key: 'سعر المنتج', value: String(productPrice ?? '') },
        { key: 'المجموع', value: String(totalPrice ?? '') }
      ],
      tags: ['COD', 'Landing Page'],
      // هذا السطر هو الأهم: بدونه سعر التوصيل يبقى مجرد نص في الملاحظات
      // ولا يُحتسب أبدًا كجزء فعلي من مجموع الطلب في شوبيفاي.
      shippingLine: {
        title: deliveryType === 'desk' ? 'توصيل للمكتب' : 'توصيل للمنزل',
