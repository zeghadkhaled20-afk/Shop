...
export default async function handler(req, res) {
  ...
  if (
    ...
    !variantId ||
    !ALLOWED_VARIANTS.includes(String(variantId))
  ) {
    return res
      .status(400)
      .json({ message: "البيانات المرسلة غير صحيحة أو غير مكتملة." });
  }

  const mutation = ` mutation draftOrderCreate($input: DraftOrderInput!) { draftOrderCreate(input: $input) { draftOrder { id name } userErrors { field message } } } `;
  ...
  try {
    ...
    return res
      .status(500)
      .json({ message: "حدث خطأ غير متوقع أثناء إرسال الطلب." });
  }
}
